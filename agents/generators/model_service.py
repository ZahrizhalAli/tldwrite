...
def validate_model_name(model_name: str) -> bool:
    ...
    # Based on openrouter for now

    return model_name in AVAILABLE_MODELS

def get_model_mapping(model_name: str) -> str:
    """Return available model mapping"""
...
async def get_model_response_async(
    ...
    try:
        mapped_model = get_model_mapping(model_name)
...
async def get_parallel_response(prompt: str, context: Dict[str, Any], model1: str, model2: str):
    """
    ...
    gen_result1 = get_model_response_async(model1, prompt, context)
    gen_result2 = get_model_response_async(model2, prompt, context)

    return gen_result1, gen_result2

def get_model_responses(prompt: str, context: Dict[str, Any], model1: str, model2: str):
    loop = asyncio.get_event_loop()

    return loop.run_until_complete(
        get_parallel_response(prompt, context, model1, model2)
    )
